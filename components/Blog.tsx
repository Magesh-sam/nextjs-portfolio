  ...
  return (
    <>
      <article key={cuid} className="">
        ...
        >
          <h2 className="mb-2 flex items-center text-3xl font-bold text-[#f1f1f1] hover:opacity-90">
            {`${title}`}
          </h2>
          <h3 className="text-base mb-2 text-gray-400">{`${views} views - ${readTimeInMinutes} min read`}</h3>
          <Image
            ...
            className=" h-auto w-full rounded-lg mb-2 hover:opacity-90"
          />
        </Link>
        <p className="text-base text-gray-300 mb-2 ">
          {`${brief}`}
          <Link
          className="ml-2 text-white  hover:opacity-90 group "
          target="_blank"
          href={`https://mageshkannan.hashnode.dev/${slug}`}
          >
            View More <span className="group-hover:pl-2 transition-all" >&rarr;</span>
        </Link>
        </p>
